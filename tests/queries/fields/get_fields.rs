...
async fn test_get_field_from_task_success() {
    ...
    let view = AddFieldToTaskQueryView::new(
        task_id,
        HashMap::from([("field1".to_string(), expected.clone())]),
    );
    ...
    assert_eq!(fields[0], expected, "Expected field to match");
}

#[tokio::test]
async fn test_get_fields_from_task_unknown_task() {
    let (_container, host) = get_shared_db().await;
    let db = get_smart_db(host.to_string()).await;

    let view = GetTaskFieldsQueryView::new(999);
    let result = db
        .fetch_one::<HashMap<String, DynamicTaskField>, _>(&view)
        .await;
